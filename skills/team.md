...
상태 기록 (`.compound/state/team-state.json`에 Write 도구로 저장):
```bash
# Write 도구로 상태 파일 생성
# 파일: .compound/state/team-state.json
```
```json
{
  "mode": "team",
...
**단계 전환 시 상태 업데이트:**
```bash
# Read: .compound/state/team-state.json 로 현재 상태 읽기
# 필요한 필드 수정 후 Write 도구로 업데이트:
# {"current_phase": "team-exec", "stage_history": "team-plan:T1,team-prd:T2,team-exec:T3"}
```

**재개 감지용 상태 읽기:**
```bash
# Read: .compound/state/team-state.json
```
...
N명의 팀원을 `Agent`로 스폰. `team_name`과 `name` 파라미터 사용. 각 팀원에게 워커 프리앰블(아래 참조) + 특정 배정 주입.
...
```bash
# 계획 후 team-exec 진입 — .compound/state/team-state.json 업데이트:
# {"current_phase": "team-exec", "stage_history": "team-plan:T1,team-prd:T2,team-exec:T3"}
...
# 검증 실패 후 team-fix 진입:
# {"current_phase": "team-fix", "fix_loop_count": 1}
```

활용:
- **재개**: 리드 크래시 시 `Read: .compound/state/team-state.json`으로 마지막 단계와 팀명 복구
...
```bash
# 팀 상태 — Write: .compound/state/team-state.json
# {"mode":"team","active":true,"current_phase":"team-plan","team_name":"build-rest-api","linked_ralph":true,"task":"REST API 전체 구축"}

# Ralph 상태 — Write: .compound/state/ralph-state.json
# {"mode":"ralph","active":true,"iteration":1,"max_iterations":10,"current_phase":"execution","linked_team":true,"team_name":"build-rest-api"}
```
...
4. 필요 시 교체 팀원 스폰: `Agent(team_name, name)`
</Error_Handling>
...
1. `Read: .compound/state/team-state.json`으로 팀 상태 읽어 `team_name`과 `linked_ralph` 확인
...
5. `Bash: rm -f .compound/state/team-state.json`으로 상태 정리
6. `linked_ralph`가 true이면 Ralph도 정리: `Bash: rm -f .compound/state/ralph-state.json`
...
- **강제 취소 (`--force`):** `Bash: rm -f .compound/state/team-state.json .compound/state/ralph-state.json`으로 양쪽 상태 모두 무조건 정리.
...
2. 상태 정리:
   ```bash
   rm -f .compound/state/team-state.json
   ```
   Ralph 연결 시:
   ```bash
