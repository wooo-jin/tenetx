...
- **거부 시**: 지적된 이슈 수정 후 같은 티어에서 재검증 (중단하지 않음). **최대 3회 Architect 재검증.** 3회 시도 후에도 거부되면 남은 이슈를 사용자에게 에스컬레이션하고 가이드를 요청합니다.
...
- Architect가 검증을 거부하면 이슈를 수정하고 재검증 (최대 3회, 초과 시 사용자에게 에스컬레이션)
</Escalation_And_Stop_Conditions>
