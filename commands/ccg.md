...
## Step 3 — 응답 수집 및 MultiModelSynthesizer 합성
...
**MultiModelSynthesizer** (`src/engine/synthesizer.ts`)를 사용하여 자동 합성:

```bash
# CLI에서 직접 합성 실행 (모든 프로바이더 병렬 호출 + 자동 합성)
tenetx ask --all "사용자 질문"

# 비교 분석 모드 (개별 점수 + 합의/차이 분석)
tenetx ask --compare "사용자 질문"
```

합성 엔진이 자동으로 수행하는 분석:
1. **응답 평가** — 각 모델 응답의 관련성, 완성도, 코드 품질, 확신도를 휴리스틱 점수화 (LLM 호출 없음)
2. **합의 분석** — 공통점(높은 신뢰도), 고유 기여(혁신 또는 오류), 모순점(해결 필요) 식별
3. **가중 합성** — 작업 유형별 프로바이더 가중치 적용 (예: 테스트 → Codex 우선, 아키텍처 → Claude 우선)
4. **전략 선택** — 합의도에 따라 자동 전략 결정:
   - 높은 합의(>80%) → 합의 기반 합성 + 고유 인사이트 보강
   - 중간 합의(40-80%) → 구조화된 비교 표시
   - 낮은 합의(<40%) → 사람 검토 권고 + 모든 옵션 제시

### 합성 출력 형식

합성 엔진이 자동 생성하는 출력에는 다음이 포함됩니다:
- 프로바이더별 점수표 (관련성, 완성도, 코드 품질, 확신도, 종합)
- 합의 사항 목록
- 모순점 분석
- 최적 프로바이더 및 가중 합성 결과
...
합성 결과는 `~/.compound/synth/`에 자동 기록되어 향후 가중치 최적화에 활용됩니다:
```bash
tenetx synth status    # 프로바이더 성능 통계
tenetx synth weights   # 작업 유형별 현재 가중치
tenetx synth history   # 최근 합성 결과
```
