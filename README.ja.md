...
- **Claude Code** がインストール・認証済みであること
  > TenetxはClaude Codeをラップし、そのHook APIに依存しています。Claude Codeの更新によりtenetxの更新が必要になる場合があります。

### Tenetxの使用に適した場面

| シナリオ | 適合度 |
|----------|--------|
| 繰り返しパターンのある長期プロジェクト | 最適 |
| 個人ワークフローの最適化 | 最適 |
| 軽量ハーネス（ランタイム依存3つ） | 最適 |
| 使い捨てスクリプトや一時的なコード | 不向き |
| Claude Codeがない環境 | サポート外 |
| チーム全体の標準化（OMC推奨） | 主目的ではない |
...
<details>
<summary>txd — パーミッションチェックのスキップ</summary>

```bash
txd                   # tenetx --dangerously-skip-permissions と同等
```

**警告**: `txd`はすべてのClaude Codeパーミッションチェックを無効にします。ツールは確認なしに実行されます。信頼できる隔離された環境でのみ使用してください。
