...
- **1일차**: 언어, 응답 스타일, 워크플로우 선호 감지
- **1주차**: 관찰된 패턴에서 `.claude/rules/` 자동 생성
...
평소처럼 코딩
    ↓
16개 훅이 조용히 관찰 (프롬프트 패턴, 도구 사용, 코드 반영)
    ↓
패턴 감지 → 솔루션 저장 → 증거 추적
    ↓
...
| mature | 0.85 | reflected >= 8, sessions >= 5, 30일 유지 |
...
35개 감지 패턴 (표면 25개 + 사고 10개) + 컨텍스트 압축 시 Claude 의미 분석.
...
| 계층 | 역할 | 구성 |
|------|------|------|
| **관찰** | 작업 방식 관찰 | 16개 훅 (compound-core 8, safety 4, workflow 4) |
| **추출** | 패턴 발견 | prompt-learner (35개 패턴) + Claude 분석 (pre-compact) |
...
| 소스 코드 | ~24K줄 |
| 테스트 | 85파일, 1,450개 |
| 활성 훅 | 16개 (19개 중, OMC에 3개 양보) |
| 감지 패턴 | 35개 (표면 25 + 사고 10) |
| MCP 도구 | 4개 |
